...
// O termo de uso do DataJud define um limite de 120 requisições por minuto.
// Um 429 ("Too Many Requests") não é uma falha real do processo consultado —
// é a própria API pedindo pra tentar de novo mais devagar. Por isso, em vez
// de desistir na hora, espera um pouco e tenta de novo (até 3 tentativas,
// com espera crescente) antes de registrar como erro de verdade.
async function consultarDatajud(tribunal: string, numeroProcesso: string, tentativa = 1): Promise<any> {
  ...
  if (resp.status === 429 && tentativa < 3) {
    await sleep(tentativa * 1500); // 1.5s, depois 3s
    return consultarDatajud(tribunal, numeroProcesso, tentativa + 1);
  }
...
//
// Responsável padrão das tarefas automáticas: Juliana Bacelar. Busca pelo
// nome (em vez de gravar um ID fixo aqui) — assim continua funcionando
// mesmo que a conta dela seja recriada um dia. Se não achar por algum
// motivo, cria sem responsável (mesmo comportamento de antes) e loga aviso.
async function buscarResponsavelPadrao(admin: ReturnType<typeof createClient>): Promise<string | null> {
...
const TAMANHO_LOTE_POR_EXECUCAO = 30;
...
async function executarSincronizacao(admin: ReturnType<typeof createClient>) {
  ...
  for (const processo of processos || []) {
    ...
    await sleep(350); // respiro entre requisições, além do retry automático em caso de 429
